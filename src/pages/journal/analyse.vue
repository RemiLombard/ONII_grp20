...
const analyzeDream = async (dreamText) => {
  const url = 'https://onii.remilombard.fr/api.together.xyz/v1/chat/completions';
      ...
      <div v-if="dream" class="mt-24 mb-10 space-y-5">
        <div class="bg-violet-950 text-white p-2.5 rounded-[15px]">
          <div class="flex items-center mb-5">
            <img :src="userProfile?.avatar ? `https://onii.remilombard.fr/api/files/${userProfile?.collectionId}/${userProfile?.id}/${userProfile?.avatar}` : 'default-avatar.png'" alt="Profile" class="w-10 h-10 rounded-full mr-3" />
            <div>
