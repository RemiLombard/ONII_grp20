...
<template>
  ...
  <div class="mb-5">
    ...
    <select id="lucide" v-model="selectedLucide" class="w-full p-2 rounded bg-nightblue text-white">
      <option value="">Aucun</option>
      <option v-for="option in yesNoOptions" :key="option" :value="option">{{ option }}</option>
    </select>
  </div>
  <div class="flex gap-2.5">
    <button @click="applyFilters" class="w-full p-3 bg-fuchsia-700 rounded-[15px] text-white mt-5">
      Filtrer
    </button>
